...
def show_move_details(item: dict | None) -> None:
    ...
    if item.get("explanation"):
        ...
        if exp.get("evaluation_human"):
            st.markdown(f"**Avaliação humana:** {exp['evaluation_human']}")
        if exp.get("situation_before"):
            st.markdown(f"**Situação antes do lance:** {exp['situation_before']}")
        st.markdown(f"**Problema do lance jogado:** {exp.get('why_it_worsens')}")
        if exp.get("best_move_value"):
            st.markdown(f"**O que o melhor lance resolvia:** {exp['best_move_value']}")
        if exp.get("opponent_plan"):
            st.markdown(f"**Plano do adversário:** {exp['opponent_plan']}")
        priorities = exp.get("position_priorities") or []
        if priorities:
            st.markdown("**Prioridade da posição:** " + " → ".join(priorities))
        comparison = exp.get("direct_comparison") or []
        if comparison:
            st.markdown("**Comparação prática**")
            st.table(
                [
                    {"Lance jogado": row.get("played"), "Melhor lance": row.get("best")}
                    for row in comparison
                ]
            )
        consequences = exp.get("practical_consequences") or []
        if consequences:
            st.markdown("**Consequências concretas:**")
    ...
    with mini_cols[0]:
        render_mini_board(item.get("fen_after"), "Após o lance jogado")
    with mini_cols[1]:
        render_mini_board(item.get("fen_best"), "Após o melhor lance")
...
def show_critical_cards(report: dict, slider_key: str | None = None) -> None:
    ...
    for card in cards:
        ...
        with st.expander(label):
            ...
            if card.get("evaluation_human"):
                st.markdown(f"**Avaliação humana:** {card.get('evaluation_human')}")
            st.markdown(f"**Resumo:** {card.get('short_reason')}")
            st.markdown(f"**Dica de estudo:** {card.get('coach_tip')}")
            if card.get("position_priorities"):
                st.markdown("**Prioridade:** " + " → ".join(card["position_priorities"]))
            ...
            if card.get("direct_comparison"):
                ...
                st.table(
                    [
                        {"Lance jogado": row.get("played"), "Melhor lance": row.get("best")}
                        for row in card["direct_comparison"]
                    ]
                )
            if card.get("practical_consequences"):
                st.markdown("**Consequências concretas:**")
                for consequence in card["practical_consequences"]:
                    st.markdown(f"- {consequence}")
            if card.get("themes"):
...
def coach_message(item: dict | None) -> str:
    ...
    summary = explanation.get("human_summary")
    if summary:
...
def render_mini_board(fen: str | None, title: str) -> None:
    if not fen:
        return
    st.caption(title)
    render_board({"board": chess.Board(fen), "lastmove": None}, size=220)
...
def critical_note(item: dict | None) -> str | None:
    ...
    explanation = item.get("explanation") or {}
    plan = explanation.get("opponent_plan") or explanation.get("best_move_value")
    suffix = f" · {plan}" if plan else ""
    return f"{CLASS_EMOJI.get(item.get('classification'), '⚠️')} Melhor: {best} · perda {loss_text}{suffix}"
