...
def _critical_card(item: dict) -> dict:
    ...
    return {
        ...
        "themes": themes,
        "short_reason": explanation.get("human_summary")
        or explanation.get("why_it_worsens")
        or f"O lance {item.get('played_san')} piorou a avaliação em relação a {best}.",
        "coach_tip": explanation.get("best_move_value")
        or explanation.get("missed_idea")
        or "Revise a posição no tabuleiro e compare com a melhor linha.",
        "position_priorities": explanation.get("position_priorities", []),
        "practical_consequences": explanation.get("practical_consequences", []),
        "opponent_plan": explanation.get("opponent_plan"),
        "direct_comparison": explanation.get("direct_comparison", []),
        "evaluation_human": explanation.get("evaluation_human"),
        "pv": item.get("pv") or [],
        "ply": item.get("ply"),
